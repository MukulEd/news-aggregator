...
class ArticleController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        $offset = $request->query('offset', $request->userPreview ? 200 : 15);
        ...
        if (($request->has('authors') && $request->filled('authors'))) { //|| $userAuthor
            $userAuthor = $request->authors;
            $response = $response->whereIn('author', $userAuthor);
        }

        if (($request->has('categories') && $request->filled('categories'))) {
            $userCategory = $request->categories;
            $response = $response->whereIn('category', $userCategory);
        }

        if (($request->has('sources') && $request->filled('sources'))) {

            $userSource = $request->sources;
